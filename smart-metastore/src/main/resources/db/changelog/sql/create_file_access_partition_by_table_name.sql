CREATE OR REPLACE FUNCTION create_file_access_partition(source_table_name text, input_date timestamp)
    RETURNS INTEGER AS '
    DECLARE
    ...
    BEGIN
        ...
        partition_table_name := FORMAT(''%s_%s'', source_table_name, current_date_part_text::TEXT);
        IF (TO_REGCLASS(partition_table_name::TEXT) ISNULL) THEN
            ...
            create_query := FORMAT(
                    ''CREATE TABLE %I PARTITION OF %I FOR VALUES FROM (extract(epoch from %L::DATE) * 1000) TO (extract(epoch from %L::DATE) * 1000);'',
                    partition_table_name, source_table_name, first_day_of_month, last_day_of_month);
