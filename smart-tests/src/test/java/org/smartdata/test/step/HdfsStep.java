...
public class HdfsStep {
  ...
  public HdfsStep rename(SsmComponent component, String srcPath, String dstPath) {
    boolean renamed = withFileSystem(component, fileSystem -> fileSystem.rename(new Path(srcPath), new Path(dstPath)));
    assertThat(renamed)
  ...
  public HdfsStep setReplication(SsmComponent component, String path, int replication) {
    withFileSystem(component, fileSystem -> fileSystem.setReplication(new Path(path), (short) replication));
    return this;
  }
  ...
  public String getPermissions(SsmComponent component, String path) {
    short permissions = withFileSystem(component,
        fileSystem -> fileSystem.getFileStatus(new Path(path)).getPermission().toOctal());
    return String.valueOf(permissions);
  }

  @Step("Get replication factor of '{path}' on {component}")
  public int getReplication(SsmComponent component, String path) {
    short replication = withFileSystem(component,
        fileSystem -> fileSystem.getFileStatus(new Path(path)).getReplication());
    return replication;
  }

  @Step("Get owner of '{path}' on {component}")
  public String getOwner(SsmComponent component, String path) {
    return withFileSystem(component, fileSystem -> fileSystem.getFileStatus(new Path(path)).getOwner());
  }

  @Step("Get group of '{path}' on {component}")
  public String getGroup(SsmComponent component, String path) {
    return withFileSystem(component, fileSystem -> fileSystem.getFileStatus(new Path(path)).getGroup());
  }
  ...
  public long getModificationTime(SsmComponent component, String path) {
    return withFileSystem(component, fileSystem -> fileSystem.getFileStatus(new Path(path)).getModificationTime());
  }
  ...
  public void checkOwner(SsmComponent component, String path, String expectedOwner, String expectedGroup) {
    runFileCheck(path, component, () -> {
      FileStatus fileStatus = withFileSystem(component, fileSystem -> fileSystem.getFileStatus(new Path(path)));
  ...
  public void checkReplication(SsmComponent component, String path, int expectedReplication) {
    runFileCheck(path,
        component,
  ...
  public HdfsStep createFileAndAwaitOn(SsmComponent component,
                                       String path,
                                       String content,
  ...
  public HdfsStep createFileOnBothClusters(SsmComponent firstComponent,
                                           SsmComponent secondComponent,
                                           String path,
                                           String content) {
  ...
  public void waitUntilFileHasOwnerAndGroup(SsmComponent component,
                                            String path,
  ...
  public void waitUntilFileHasReplication(SsmComponent component, String path, int expectedReplication) {
    waitUntil(() -> checkReplication(component, path, expectedReplication), SYNC_ACTION_TIMEOUT);
  }

  @Step("Wait until file '{path}' has modification time {expectedModificationTime} on {component}")
  public void waitUntilFileHasModificationTime(SsmComponent component,
                                               String path,
  ...
  public void waitUntilFileModificationTimeAfter(SsmComponent component, String path, long time) {
    waitUntil(() -> runFileCheck(path,
        component,
        () -> assertThat(getModificationTime(component, path))
            .as("Modification time of file '%s' on %s", path, component)
            .isGreaterThan(time)), SYNC_ACTION_TIMEOUT);
  }

  private void runFileCheck(String path, SsmComponent component, Runnable check) {
    try {
      check.run();
    } catch (UncheckedIOException e) {
      throw new AssertionError(
          format("File '%s' is not available on %s: %s", path, component, e.getMessage()),
          e);
    }
  }
  ...
  private <T> T withFileSystem(SsmComponent component, FileSystemOperation<T> operation) {
    try {
      ...
      throw new UncheckedIOException(
          format("HDFS operation on %s (%s) failed", component, getFileSystemUri(component)),
          e);
    }
  }
  ...
  private FileSystem getFileSystem(SsmComponent component) throws IOException {
    ...
    try {
      ...
      return FileSystem.get(URI.create(uri), conf, hdfsUser);
