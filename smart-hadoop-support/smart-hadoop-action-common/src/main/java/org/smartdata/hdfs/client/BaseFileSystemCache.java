...
@Slf4j
public abstract class BaseFileSystemCache<T extends FileSystem> implements FileSystemCache<T> {
  ...
  public T get(Configuration config, String user, InetSocketAddress ssmMasterAddress)
      throws IOException {
    CacheKey cacheKey = new CacheKey(ssmMasterAddress, user, getServiceUri(config));
  ...
  protected abstract URI getServiceUri(Configuration config) throws IOException;
